...
## 18. Context Management (CRITICAL)

> Относится ТОЛЬКО к основной сессии Хайзенберга. Sub-агенты не применяют.

Check context % every 5-7 responses via `session_status`.

| Context | Action |
|---------|--------|
| >60% | Delegate ALL heavy work to subagents |
| >70% | **STOP.** Write handoff + diary FIRST. Then continue |
| >75% | Alert user: "⚡ Context 75%" |
| >80% | **FULL STOP.** Write handoff → propose /new |

**Forbidden in main session:**
- `config.get` without specific `path` (full config = ~50K tokens)
- Research with >3 web_search → subagent
- Reading files >100 lines → subagent
- Any analysis with 3+ tool calls → subagent

**DO NOT wait for automatic compaction!** Handoff before it is always better.
