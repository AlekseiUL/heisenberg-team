...
### Агенты:
- Маркетинг, воронки, CTR → маркетолог (`marketing-funnel`)
- Код, файлы, сложные задачи → тимлид (`teamlead`)
- Цели, привычки, Obsidian → кайдзен (`kaizen`, Sonnet)
- Контент, ОТК, доставка → продюсер (`producer`). {{OWNER_NAME}} пишет продюсеру напрямую. Главный агент НЕ участвует (только эскалации)
